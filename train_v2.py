...
class DualImagePairDataset(Dataset):
    def __init__(self, masked_root_dir, unmasked_root_dir):
        self.masked_root_dir = masked_root_dir
        self.unmasked_root_dir = unmasked_root_dir
        self.subfolders = [f for f in os.listdir(masked_root_dir) if os.path.isdir(os.path.join(masked_root_dir, f))]
        
        self.image_pairs = []
        for subfolder in self.subfolders:
            masked_path = os.path.join(masked_root_dir, subfolder)
            unmasked_path = os.path.join(unmasked_root_dir, subfolder)
            print(f"Checking masked path: {masked_path}")
            print(f"Checking unmasked path: {unmasked_path}")
            
            # Look for .jpg files instead of .png
            masked_images = sorted([os.path.join(masked_path, img) for img in os.listdir(masked_path) if img.endswith('.jpg')])
            unmasked_images = sorted([os.path.join(unmasked_path, img) for img in os.listdir(unmasked_path) if img.endswith('.jpg')])
            
            print(f"Masked images found: {len(masked_images)}")
            print(f"Unmasked images found: {len(unmasked_images)}")
            
            if len(masked_images) == len(unmasked_images) and len(masked_images) > 1:
                img_a_path = masked_images[0]
                for i in range(1, len(masked_images)):
                    masked_img_b_path = masked_images[i]
                    unmasked_img_b_path = unmasked_images[i]
                    self.image_pairs.append((img_a_path, masked_img_b_path, unmasked_img_b_path))
                print(f"Added {len(masked_images) - 1} pairs from subfolder {subfolder}")

    def __len__(self):
        return len(self.image_pairs)

    def __getitem__(self, idx):
        img_a_path, masked_img_b_path, unmasked_img_b_path = self.image_pairs[idx]
        
        img_a = cv2.imread(img_a_path)
        masked_img_b = cv2.imread(masked_img_b_path)
        unmasked_img_b = cv2.imread(unmasked_img_b_path)
        
        img_a = cv2.cvtColor(img_a, cv2.COLOR_BGR2RGB)
        masked_img_b = cv2.cvtColor(masked_img_b, cv2.COLOR_BGR2RGB)
        unmasked_img_b = cv2.cvtColor(unmasked_img_b, cv2.COLOR_BGR2RGB)
        
        return img_a, masked_img_b, unmasked_img_b
...
class RBSR_X(nn.Module):
    ...
    def forward(self, past_frame, curr_frame, True_curr_frame):
        first_img_a, first_img_b, ground_truth_img = past_frame.to(device), curr_frame.to(device), True_curr_frame.to(device)
        ...
        ground_truth_transposed = ground_truth_img.permute(0, 3, 1, 2).float()  # Shape becomes (1, 3, 1080, 1920)

        # Resize the image using interpolate (bicubic by default, can change mode if needed)
        ground_truth_resized = F.interpolate(ground_truth_transposed, size=(2160, 3840), mode='bilinear', align_corners=False)

        # Ensure the tensor is float (if not already)
        ground_truth_resized = ground_truth_resized.float()
        synthetic_tensor_temp = synthetic_tensor.unsqueeze(0).float()

        # Move the input tensor to the same device
        ground_truth_resized = ground_truth_resized.to(device)
        synthetic_tensor_temp = synthetic_tensor_temp.to(device)

        D_real = self.discriminator(ground_truth_resized)  # Discriminator output for real images
...
if __name__ == '__main__':
    ...
    masked_root_dir = r"C:\Users\User\Desktop\pure_clear_series"
    unmasked_root_dir = r"D:\Desmoking_Dataset\LH_frames\1970_01_01_010226_LH_S6\pure_clear_series"
    dataset = DualImagePairDataset(masked_root_dir, unmasked_root_dir)
    ...
    for epoch in range(num_epochs):
        ...
        running_loss = 0.0
        for img_a, masked_img_b, unmasked_img_b in pbar:  # Updated to handle three images
            # Move tensors to device (already PyTorch tensors from DataLoader)
            img_a = img_a.to(device)
            masked_img_b = masked_img_b.to(device)
            unmasked_img_b = unmasked_img_b.to(device)

            optimizer.zero_grad()

            # Forward pass - assuming model accepts img_a and masked_img_b as inputs
            # and uses unmasked_img_b internally for ground truth comparison
            synthetic_tensor, l1_loss, Rec_Loss, loss_D, loss_G = model(img_a, masked_img_b, unmasked_img_b)
