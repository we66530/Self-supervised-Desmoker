...
# Global variables to store paths and images
guide_img = None
input_img = None
model = None
...
class ImageProcessingApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Image Desmoking Tool")

        # Labels to display file paths
        self.guide_label = tk.Label(root, text="Guide Image: Not selected")
        self.guide_label.pack(pady=5)
        
        self.input_label = tk.Label(root, text="Input Image: Not selected")
        self.input_label.pack(pady=5)
        
        self.model_label = tk.Label(root, text="Model: Not selected")
        self.model_label.pack(pady=5)

        # Buttons
        tk.Button(root, text="Load Guide Image", command=self.load_guide_image).pack(pady=5)
        tk.Button(root, text="Load Input Image", command=self.load_input_image).pack(pady=5)
        tk.Button(root, text="Load Model", command=self.load_model).pack(pady=5)
        tk.Button(root, text="Run Prediction", command=self.run_prediction).pack(pady=10)

        # Canvas to display result (960x540)
    ...
    def load_guide_image(self):
        ...
        if file_path:
            guide_img = load_image(file_path).to(device)
            self.guide_label.config(text=f"Guide Image: {file_path}")

    def load_input_image(self):
        global input_img
        file_path = filedialog.askopenfilename(filetypes=[("Image files", "*.jpg *.png *.jpeg")])
        if file_path:
            input_img = load_image(file_path).to(device)
            self.input_label.config(text=f"Input Image: {file_path}")
    ...
    def load_model(self):
        ...
        if file_path:
            ...
            model.eval()
            self.model_label.config(text=f"Model: {file_path}")

    def run_prediction(self):
        global guide_img, input_img, model
        if guide_img is None or input_img is None or model is None:
            tk.messagebox.showerror("Error", "Please load both images and the model first!")
            return

        # Make prediction
        with torch.no_grad():
            synthetic_tensor = model(input_img, guide_img)

        # Post-process the output
        synthetic_tensor = synthetic_tensor.squeeze(0)  # Remove batch dimension
        synthetic_tensor = synthetic_tensor.cpu().numpy()  # Convert to numpy
        synthetic_tensor = np.transpose(synthetic_tensor, (1, 2, 0))  # [C, H, W] to [H, W, C]
        synthetic_image = ((synthetic_tensor - synthetic_tensor.min()) / 
                          (synthetic_tensor.max() - synthetic_tensor.min()) * 255).astype(np.uint8)

        # Convert to PIL and resize to 960x540
        image_pil = Image.fromarray(synthetic_image)
        image_pil = image_pil.resize((960, 540), Image.Resampling.LANCZOS)  # Resize to 960x540
        photo = ImageTk.PhotoImage(image_pil)

        self.canvas.delete("all")  # Clear previous image
        self.canvas.create_image(480, 270, image=photo)  # Center the image (960/2, 540/2)
